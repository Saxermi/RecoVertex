...
void SoAToRecoVertexProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  VertexProtoType prototypes;
  for (int iV = 0; iV < 512; iV++) {
    if (not(hostVertexView[iV].isGood()))
      continue;
    std::cout << "SoAToRecoblabla " << iV <<" z=" << hostVertexView[iV].z() << "  rho=" <<  hostVertexView[iV].rho()  <<  " nV=" <<  hostVertexView[iV].nV() << std::endl;
